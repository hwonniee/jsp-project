...
public class BbsListAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String url = "board/bbsList.jsp";
		
		BbsDAO bDao = BbsDAO.getInstance();
		
		// 게시판 전체 리스트 가져옴
		//List<BbsVO> boardList = bDao.selectAllBoards();
		...
		// 페이징 리스트를 가져옴
		List<BbsVO> bbsList = bDao.selectBoardsPaging(offset, pageSize, searchType, searchText);
		
		// bbsList를 bbsList.jsp에 전달하기 위해 설정
		request.setAttribute("bbsList", bbsList);
		request.setAttribute("pageHandler", pageHandler);
		request.setAttribute("searchVO", searchVo);
		
		// foward를 통해 url(/board/bbsList.jsp)로 이동
